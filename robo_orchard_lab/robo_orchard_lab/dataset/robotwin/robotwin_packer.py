...
class RobotwinDataPacker(BaseLmdbManipulationDataPacker):
    ...
    def input_path_handler(self, input_path):
        ...
        # Limit the number of episodes if max_episodes is specified
        if self.max_episodes is not None:
            episodes = episodes[:self.max_episodes]
            logger.info(f"Limited to {self.max_episodes} episodes")
    ...
    def _pack(self):
        ...
        for ep_id, ep in enumerate(self.episodes):
            ...
            with h5py.File(ep_path, "r") as ep_file:
                ...
                left_endpose = ep_file["endpose/left_endpose"][:]
                right_endpose = ep_file["endpose/right_endpose"][:]

                cartesian_positions = np.concatenate([left_endpose, right_endpose], axis=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_episodes", type=int, default=None, help="Maximum number of episodes to pack")
