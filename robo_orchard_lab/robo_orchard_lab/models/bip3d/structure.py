...
class BIP3D(ModelMixin):
    ...
    def extract_feat(self, inputs):
        imgs = inputs.get(self.input_2d) # 获取imgs,(B,相机通道数,RGB通道数,H,W)
        if imgs.dim() == 5:
...
class BIP3DConfig(TorchModuleCfg[BIP3D]):
    ...
    backbone: MODULE_TPYE  # type: ignore # SwinTransformer
    decoder: MODULE_TPYE  # type: ignore # SEMActionDecoder
    neck: MODULE_TPYE | None = None # ChannelMapper
    text_encoder: MODULE_TPYE | None = None # BertModel
    feature_enhancer: MODULE_TPYE | None = None # TextImageDeformable2DEnhancer
    spatial_enhancer: MODULE_TPYE | None = None # DepthFusionSpatialEnhancer
    data_preprocessor: MODULE_TPYE | None = None # BaseDataPreprocessor
    backbone_3d: MODULE_TPYE | None = None # ResNet
    neck_3d: MODULE_TPYE | None = None # ChannelMapper
    input_2d: str = "imgs"
    input_3d: str = "depths"
    embed_dims: int = 256 # embed_dims=256
