...
class SEMPolicy:
    def __init__(self, ckpt_path, config_file="config_sem_robotwin.py"):
        ...
        # update path when eval
        if not os.path.exists(self.cfg.config["urdf"]):
            self.cfg.config["urdf"] = "assets/embodiments/aloha-agilex/urdf/arx5_description_isaac.urdf"
        
        if not os.path.exists(self.cfg.config["checkpoint"]):
            self.cfg.config["checkpoint"] = "policy/SEM/pre_ckpt/groundingdino_swint_ogc_mmdet-822d7e9d-rename.pth"

        if not os.path.exists(self.cfg.config["bert_checkpoint"]):
            self.cfg.config["bert_checkpoint"] = "policy/SEM/pre_ckpt/bert-base-uncased"
